...
class HeatKappaInverseAdapter:
    """Adapter for kappa inversion from compact field-decay observations."""

    name = "heat_kappa_inverse"
    required_fields = ("decay_rate_l2", "mean_abs_t1", "mean_abs_t2", "kappa")
    infer_required_fields = ("decay_rate_l2", "mean_abs_t1", "mean_abs_t2")
    feature_names = ("decay_rate_l2", "mean_abs_t1", "mean_abs_t2")
    target_names = ("kappa",)

    def to_state(self, record: Record) -> State:
        ensure_required_fields(record, self.required_fields)
        return {
            "decay_rate_l2": float(record["decay_rate_l2"]),
            "mean_abs_t1": float(record["mean_abs_t1"]),
            "mean_abs_t2": float(record["mean_abs_t2"]),
